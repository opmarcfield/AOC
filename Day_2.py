def analyze_games(text, max_colors):
    # Splitting the text into individual games
    games = text.split("Game ")[1:]  # Starting from 1 to skip the empty string before the first "Game"

    # Splitting the max_colors string into a dictionary
    max_colors_dict = {}
    for color_max in max_colors.split(", "):
        color, max_value = color_max.split()  # Splitting each "red 8" into "red" and "8"
        max_colors_dict[color] = int(max_value)  # Converting "8" to an integer

    # Function to parse a single game's text
    def parse_game(game_text):
        rounds = game_text.split(";")
        for round in rounds:
            ...
            # If any round does not meet the criteria, return False immediately
            if not all(color_counts.get(color, 0) <= max_colors_dict.get(color, float('inf')) for color in color_counts):
                return False
        return True
    ...
    valid_games = []
    for game in games:
        game_number, game_text = game.split(":", 1)
        if parse_game(game_text):
            valid_games.append("Game " + game_number.strip())

    sum_of_game_numbers = sum(int(game.split()[1]) for game in valid_games)
    print(f"Sum of valid game numbers: {sum_of_game_numbers}")
    return valid_games
# Example usage
text = """
...
analyze_games(text, max_colors)
valid_games = analyze_games(text, max_colors)
