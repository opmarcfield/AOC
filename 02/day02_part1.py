def minimum_cubes_and_power(text):
    games = text.split("Game ")[1:]  # Splitting the text into individual games
    ...
    for game in games:
        ...
        # Initialize minimum counts with zero
        ...
        for round in rounds:
            ...
            # Update the minimum counts if current round has higher counts
            for color in color_counts:
                min_counts[color] = max(min_counts[color], color_counts[color])

        # Calculate the power of the game
        game_power = min_counts['red'] * min_counts['green'] * min_counts['blue']
...
"""
print(minimum_cubes_and_power(text))
